---
name: minions-orders
description: Agent skills for working with Minions Orders MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-orders data.
---

# Minions Orders Agent Skills

Skills for agents operating on the `minions-orders` toolbox.
...
The `orders` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-orders/sdk/schemas';
...
```python
from minions_orders.schemas import custom_types
...
- This agent only writes to `minions-orders` — it reads from other toolboxes but never writes to them
